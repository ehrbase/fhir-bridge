...
Force Tags              procedure_create   invalid    create
...
002 Create Procedure (Invalid/Missing 'code')
...
003 Create Procedure (Invalid/Missing 'category')
	[Documentation]     1. *CREATE* new an EHR record\n\n 
	...                 2. *LOAD* _create-procedure.json_\n\n
	...                 3. *UPDATE* ``Subject - Identifier - value`` with the _UUID:_ ${subject_id} which was created in EHR record\n\n
	...                 4. *UPDATE* values for attribute ``resourceType`` \n\n
	...
	# 														CODE
    #invalid coding
	$.category.coding    			missing					422
	$.category.coding    			${EMPTY}				422
	$.category.coding				${{ [] }}				422
	$.category.coding				${{ {} }}				422
	$.category.coding				${{ [{}] }}				422

	#invalid code 0
	$.category.coding[0].code    	${EMPTY}    	    	422
	$.category.coding[0].code    	${randinteger}	    	422
    $.category.coding[0].code 		${{ [] }}				422
	$.category.coding[0].code 		${{ {} }}				422
	$.category.coding[0].code 		${{ [{}] }}				422

	# invaild system 0
	$.category.coding[0].system    	${EMPTY}    	    	422
	$.category.coding[0].system    	${randstring}	    	422
	$.category.coding[0].system    	${randinteger}	    	422
	$.category.coding[0].system    	http://foobar.de      	422
    $.category.coding[0].system 	${{ [] }}				422
	$.category.coding[0].system 	${{ {} }}				422
	$.category.coding[0].system 	${{ [{}] }}				422



004 Create Procedure (Invalid/Missing 'resourceType')
	[Documentation]     1. *CREATE* new an EHR record\n\n 
	...                 2. *LOAD* _create-procedure.json_\n\n
	...                 3. *UPDATE* ``Subject - Identifier - value`` with the _UUID:_ ${subject_id} which was created in EHR record\n\n
	...                 4. *UPDATE* values for attribute ``resourceType`` \n\n
...
create procedure with ehr reference
	[Arguments]         ${json_path}        ${value}                 ${http_status_code}

						ehr.create new ehr                      000_ehr_status.json
	${payload}=    		generate payload from example json      ${json_path}    ${value}
						procedure.POST /Procedure  				Procedure  ${payload}          	
						procedure.validate response - 422 (w/o error message)  ${http_status_code}



create procedure w/o ehr reference    
	[Arguments]         ${json_path}        ${value}                ${http_status_code}
...
generate payload from example json
	...
	${payload}          Load JSON From File    ${DATA_SET_PATH_PROCEDURE}/create-procedure.json
