...
*** Test Cases ***
001 Create FiO2 (Invalid/Missing EHR Reference)
	...
	# 												INDEX	CODE
	$.subject.identifier.value		missing			0		422		mii-reference-1: Either reference.reference XOR reference.identifier exists ..this.reference.exists.. xor ..this.identifier.value.exists.. and .this.identifier.system.exists
	$.subject.identifier.system		missing			0		422		mii-reference-1: Either reference.reference XOR reference.identifier exists ..this.reference.exists.. xor ..this.identifier.value.exists.. and .this.identifier.system.exists
...
003 Create FiO2 (Invalid/Missing 'meta')
	[Documentation]		1) Create an EHR record
	...					2) Replace {{patientID}} in example JSON with EHR reference
	...					3) Generate (invalid) payload for FiO2 profile
	...					   based on example JSON and data in TC table
...
004 Create FiO2 (Invalid/Missing 'code')
	[Documentation]		1) Create an EHR record
	...					2) Replace {{patientID}} in example JSON with EHR reference
	...					3) Generate (invalid) payload for FiO2 profile
	...					   based on example JSON and data in TC table
...
005 Create FiO2 (Invalid/Missing 'category:VSCat')
	[Documentation]		1) Create an EHR record
	...					2) Replace {{patientID}} in example JSON with EHR reference
	...					3) Generate (invalid) payload for FiO2 profile
	...					   based on example JSON and data in TC table
	...
	$.category[0].coding[0].code    		${EMPTY}    	0    	422    	@value cannot be empty
	...
	$.category[0].coding[0].system    		missing    		0    	422    	A code with no system has no defined meaning. A system should be provided
	...																		Observation.category[0].coding[0]

	$.category[0].coding[0].system    		${EMPTY}    	1    	422    	@value cannot be empty
	...																		Observation.category[0].coding[0].system

	$.category[0].coding[0].system    		foobar    		0    	422    	Coding.system must be an absolute reference, not a local reference
	...																		Observation.category[0].coding[0]
...
006 Create FiO2 (Invalid/Missing 'component:SystolicBP')
	[Documentation]     TODO
	[Template]			Create FiO2 with ehr reference
    [Tags]              xxx
...
# 007 Create FiO2 (Invalid/Missing 'component:DiastolicBP')
# 	[Documentation]     TODO
# 	[Template]			Create FiO2 with ehr reference
#     [Tags]              
