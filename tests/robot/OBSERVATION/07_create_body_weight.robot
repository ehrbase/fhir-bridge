...
*** Variables ***
${body_height_url}			https://www.netzwerk-universitaetsmedizin.de/fhir/StructureDefinition/body-weight
...
001 Create Body Weight (Invalid/Missing EHR reference/subject)
    ...
    $.subject						missing			422		Observation.subject: minimum required = 1, but only found 0 .from  ${body_height_url}
...
#Therefore, some cases where value = missing do not result in errors. It seems as if the actual profile url is not checked.
	...
	#$.identifier			missing		   422     Observation.identifier:analyseBefundCode: minimum required = 1, but only found 0 .from  ${body_height_url} 
	...
	#$.identifier[0].type	missing		   422     Observation.identifier:analyseBefundCode: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	#$.identifier[0].type.coding[0].system   http://foo    	   422    This element does not match any known slice defined in the profile  ${body_height_url} -- No error message ??
	#$.identifier[0].type.coding[0].code     missing    	   422    This element does not match any known slice defined in the profile  ${body_height_url} 
	...
	#$.identifier[0].type.coding[0].code     foo    	       422    This element does not match any known slice defined in the profile  ${body_height_url}
...
006 Create Body Weight (Invalid/Missing 'code')
	...
	#											HTTP	
	# FIELD/PATH 				VALUE			CODE	ERROR MESSAGE
	$.code						missing			422    	Observation.code: minimum required = 1, but only found 0 .from  ${body_height_url}
	
	# comment: code:loinc
	$.code.coding[0].code		missing			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	...
	$.code.coding[0].code		foo			 	422    	This element does not match any known slice defined in the profile  ${body_height_url}
	...
	$.code.coding[0].system		http://foo	 	422    	This element does not match any known slice defined in the profile  ${body_height_url}
...
007 Create Body Weight (Invalid/Missing 'category')
	...
	$.category						  	missing				422    	Observation.category: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	$.category[0].coding[0].code      	missing    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	$.category[0].coding[0].code      	${EMPTY}    		422    	@value cannot be empty
	$.category[0].coding[0].code      	foobar    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	$.category[0].coding[0].system    	missing    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	...
	$.category[0].coding[0].system    	http://foo    		422		This element does not match any known slice defined in the profile  ${body_height_url}
	...
	#$.category[0].coding[1].code      	missing    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	#$.category[0].coding[1].code      	${EMPTY}    		422    	@value cannot be empty
	#$.category[0].coding[1].code      	foobar    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	#$.category[0].coding[1].code      	${123}    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	#$.category[0].coding[1].system    	missing    			422    	This element does not match any known slice defined in the profile  ${body_height_url}
	#$.category[0].coding[1].system    	${EMPTY}    		422    	Observation.category.coding:observation-category: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	#$.category[0].coding[1].system    	http://foo bar    	422  	Observation.category.coding:observation-category: minimum required = 1, but only found 0 .from  ${body_height_url}
	#$.category[0].coding[1].system    	http://foo    		422    	This element does not match any known slice defined in the profile  ${body_height_url}
...
008 Create Body Weight (Invalid/Missing 'effectiveDateTime')
	...
	#											HTTP	
	# FIELD/PATH 				VALUE			CODE	ERROR MESSAGE
	$.effectiveDateTime 		missing			422		Observation.effective.x.: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.effectiveDateTime 		${EMPTY}		422		@value cannot be empty    Observation.effective.ofType.dateTime
	$.effectiveDateTime 		${None}			422		Observation.effective.x.: minimum required = 1, but only found 0 .from  ${body_height_url}
...
009 Create Body Weight (Invalid/Missing 'valueQuantity')
	...
	$.valueQuantity			  	${{ {} }}		422    	Observation.value.x.:valueQuantity.value: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity			  	${{ {} }}		422    	Observation.value.x.:valueQuantity.unit: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity			  	${{ {} }}		422    	Observation.value.x.:valueQuantity.system: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity			  	${{ {} }}		422    	Observation.value.x.:valueQuantity.code: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity.value	  	missing			422    	Observation.value.x.:valueQuantity.value: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity.unit	  	missing			422    	Observation.value.x.:valueQuantity.unit: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity.system	  	missing			422    	Observation.value.x.:valueQuantity.system: minimum required = 1, but only found 0 .from  ${body_height_url}
	$.valueQuantity.code	  	missing			422    	Observation.value.x.:valueQuantity.code: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	$.valueQuantity.unit	  	${None}			422    	Observation.value.x.:valueQuantity.unit: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	$.valueQuantity.system	  	${None}			422    	Observation.value.x.:valueQuantity.system: minimum required = 1, but only found 0 .from  ${body_height_url}
	...
	$.valueQuantity.code	  	${None}			422    	Observation.value.x.:valueQuantity.code: minimum required = 1, but only found 0 .from  ${body_height_url}
