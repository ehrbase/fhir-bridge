...
validate response - 201
    [Documentation]     Validates response of POST to /Immunization endpoint
...
create history of vaccination
    [Arguments]         ${profil_name}    ${example_json}
    POST /Immunization with ehr reference    ${profil_name}    ${example_json}
...
POST /Immunization
    ...
    Log To Console      POSTING '${{ $fhir_resource_name.upper() }}' IMMUNIZATION
    &{resp}             POST    ${BASE_URL}/Immunization    body=${payload}
                        Output Debug Info To Console


POST /Immunization with ehr reference
    [Arguments]         ${fhir_resource_name}    ${example_json}

    ${payload}          Load JSON From File    ${DATA_SET_PATH_IMMUNIZATION}/${example_json}
                        #Update Value To Json    ${payload}    $.subject.identifier.value    ${subject_id}
                        #Output Debug Info To Console    ${payload}
                        POST /Immunization    ${fhir_resource_name}    ${payload}