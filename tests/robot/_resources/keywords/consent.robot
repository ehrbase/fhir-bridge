...
validate response - 201
    ...
    String     response body resourceType    Consent
...
create Consent
    ...
    POST /Consent with ehr reference    ${text}    ${example_json}
...
POST /Consent
    ...
    &{resp}             POST    ${BASE_URL}/Consent    body=${payload}
                        Output Debug Info To Console


POST /Consent with ehr reference
    [Arguments]         ${fhir_resource_name}    ${example_json}

    ${payload}          Load JSON From File    ${DATA_SET_PATH_CONSENT}/${example_json}
                        Update Value To Json    ${payload}    $.patient.identifier.value    ${subject_id}
                        Output Debug Info To Console    ${payload}
                        POST /Consent    ${fhir_resource_name}    ${payload}


POST /Consent w/o ehr reference
