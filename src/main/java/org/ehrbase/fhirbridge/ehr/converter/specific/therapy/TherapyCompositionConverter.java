...
public class TherapyCompositionConverter extends CompositionConverter<Procedure, GECCOProzedurComposition> {
    ...
    protected GECCOProzedurComposition convertInternal(Procedure resource) {
        ...
        switch (resource.getStatus()) {
            case UNKNOWN:
                ...
                break;
            case NOTDONE:
                mapNotDone(resource, result);
                break;
            case ENTEREDINERROR:
                throw new ConversionException("Invalid status");
            default:
                mapDone(resource, result);
        }
